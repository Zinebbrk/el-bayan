...
def initialize_pipeline():
    """Initialize pipeline (can be called lazily)"""
    global pipeline, pipeline_initializing, pipeline_error
    ...
    if pipeline_error:
        raise HTTPException(status_code=503, detail=f"Pipeline initialization failed: {pipeline_error}")
...
async def startup_event():
    """Start API server (pipeline loads lazily)"""
    logger.info("API server starting...")
    logger.info("Pipeline will initialize on first request")
    # Don't initialize pipeline here - do it lazily on first request
